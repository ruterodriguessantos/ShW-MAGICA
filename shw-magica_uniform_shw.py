...
--> Uniform electric field implementation with explicit shield wire modelling <--
...
def input_output(X):
    ...
    shw_networkpath = "Data/GRID_ShW.txt"
    shw_connectionspath =  'Data/CONNECTIONS_ShW.txt'
    
    temp=connectionspath.split('/')
    filename1=temp[-1]
    SavePath=connectionspath[0:-len(filename1)]
         
    out_name         = 'uniform_Efield_with_shw_'
    
    return testfield,networkpath,connectionspath,shw_networkpath, shw_connectionspath,out_name, SavePath;
...
testfield,networkpath,connectionspath,shw_networkpath,shw_connectionspath,out_name,SavePath     =input_output('x')
...
# ===============================================================
 # !! New block, to account for new connections and new nodes related to the shield wire circuits !!

nsw_half=len([i for i in range(len(substation)) if '-' in substation[i]])
nsw=2*nsw_half  
print(nsw)

substation_E = []
nodefrom_E = np.zeros(nsw, dtype=np.int32)
nodeto_E = np.zeros(nsw, dtype=np.int32)
res_line_E = np.zeros(nsw, dtype=npf)
voltage_lines_E = np.zeros(nsw, dtype=np.int32)
line_num_E = np.zeros(nsw, dtype=np.int32)
mother_line_E = np.zeros(nsw, dtype=np.int32)
RG = np.zeros(nsw, dtype=npf)
Req = np.zeros(nsw, dtype=npf)

sitenum_E = np.zeros(nsw, dtype=np.int32)
sitename_E = []
geolat_E, geolon_E = np.zeros(nsw, dtype=npf), np.zeros(nsw, dtype=npf)
res_earth_E = np.zeros(nsw, dtype=npf)
res_trans_E = np.zeros(nsw, dtype=npf)

# READ GRID DATA:
# ---------------------
shw_network = open(shw_networkpath, 'r')
shw_netdata = shw_network.readlines()

# Define number of nodes:
nnodes_E = len(shw_netdata)

for i in range(nnodes_E):
    # READ NODE DATA:
    # ---------------
    data = shw_netdata[i].split(" ")
    # Dictionary for station to index:
    # In this way, the index for stations starts in '0' and ends in 'nnodes-1' 
    s2i[data[1]] = int(data[1]) - 1
    # Dictionary for index to station:
    i2s[int(data[1]) - 1] = data[1]
    # Site number:
    sitenum_E[i] = int(data[1]) -1         # -1 to simplify python indices
    # Site readable names:
    sitename_E.append(data[0])
    # Geographic latitude of node:
    geolat_E[i] = float(data[2])
    # Geographic longitude of node:
    geolon_E[i] = float(data[3])
    # Earthing resistance of each node:
    res_earth_E[i] = float(data[4])
    # Transformer resistances (see notes):
    res_trans_E[i] = float(data[5])

# READ CONNECTION DATA:
# ---------------------
shw_connectionsfile = open(shw_connectionspath, 'r')
shw_conndata = shw_connectionsfile.readlines()

# Number of connections:
nconnects_E = len(shw_conndata)

for i in range(0, nconnects_E):
    conns = shw_conndata[i].split(" ")
    # Substation name:
    substation_E.append (conns[0])
    # Connection starts at site:
    nodefrom_E[i] = int(s2i[conns[2]])
    # Connection ends at site:
    nodeto_E[i] = int(s2i[conns[3]])
    # Line resistance between connecting sites:
    res_line_E[i] = float(conns[4])
    # Voltage level of line:
    voltage_lines_E[i] = 888
    # Number line
    line_num_E[i] = int(conns[1])
    # Number line from which this shield wire line was created:
    mother_line_E[i] = int(conns[5])
    # Value RG:
    RG[i] = float(conns[6])
    # Value Req:
    Req[i] = float(conns[7].split("\n")[0])

# Set inf values to very high resistance for computation:
res_earth_E[res_earth_E==np.inf] = 0
res_line_E[res_line_E==np.inf] = 0

# !! New block, to account for new connections and new nodes related to the shield wire circuits !!
#
# ===============================================================
# 2) CONCATENATE REAL CONNECTIONS AND NODES WITH SHIELD-WIRE CONNECTIONS AND NODES
# ===============================================================

# Needs to define new, larger matrices
# res_earth_big, res_trans_big, geolat_big, geolon_big, res_line_big, nodefrom_big, nodeto_big
#
nnodes_new = nnodes + nsw
nconnects_new = nconnects + nsw

nodefrom_big = np.zeros(nconnects_new, dtype=np.int32)
nodeto_big = np.zeros(nconnects_new, dtype=np.int32)
res_line_big = np.zeros(nconnects_new, dtype=npf)
voltage_lines_big = np.zeros(nconnects_new, dtype=np.int32)
sitename_big = []
substation_big = []


geolat_big, geolon_big = np.zeros(nnodes_new, dtype=npf), np.zeros(nnodes_new, dtype=npf)
res_earth_big = np.zeros(nnodes_new, dtype=npf)
res_trans_big = np.zeros(nnodes_new, dtype=npf)

for i in range(0,nnodes):
    res_earth_big[i] = res_earth[i]
    res_trans_big[i] = res_trans[i]
    geolat_big[i] = geolat[i]
    geolon_big[i] = geolon[i]
    sitename_big.append(sitename[i])
    print(i,sitename[i])

for i in range(nnodes,nnodes_new):
    res_earth_big[i] = res_earth_E[i-nnodes]
    res_trans_big[i] = res_trans_E[i-nnodes]
    geolat_big[i] = geolat_E[i-nnodes]
    geolon_big[i] = geolon_E[i-nnodes]
    sitename_big.append(sitename_E[i-nnodes]) 
    print(i,sitename_E[i-nnodes])

for i in range(0,nconnects):
    res_line_big[i] = res_line[i]  
    nodefrom_big[i] = nodefrom[i]
    nodeto_big[i] = nodeto[i]
    voltage_lines_big[i] = voltage_lines[i]
    substation_big.append(substation[i])

for i in range(nconnects,nconnects_new):
    res_line_big[i] = res_line_E[i-nconnects]
    nodefrom_big[i] = nodefrom_E[i-nconnects]
    nodeto_big[i] = nodeto_E[i-nconnects]
    voltage_lines_big[i] = voltage_lines_E[i-nconnects]
    substation_big.append(substation_E[i-nconnects])
...
resis = np.zeros((nnodes_new, nnodes_new), dtype=npf)
connections = np.zeros((nnodes_new,nnodes_new), dtype=npf)

# I changed a bit the original if statement, in order to guarantee that matrix resis
# is filled symmetrically
#               A.P.

for i in range(0,nconnects_new):
    x, y = int(nodefrom_big[i]), int(nodeto_big[i])
    if (resis[x,y] > 0. and res_line_big[i] > 0.):
        # If res already added to this line, add another line in parallel:
        resis[x,y] = 1./(1./resis[x,y] + 1./res_line_big[i])
        resis[y,x] = resis[x,y]
    else:
        resis[x,y] = res_line_big[i]
        resis[y,x] = resis[x,y]
    if resis[x,y] > 0.:
...
dists = np.zeros((nnodes_new, nnodes_new), dtype=npf)
azi = np.zeros((nnodes_new, nnodes_new), dtype=npf)
...
for i in range(0,nnodes_new):
    for j in range(0,nnodes_new):
        lati, loni = geolat_big[i], geolon_big[i]
        latj, lonj = geolat_big[j], geolon_big[j]
        if resis[i, j] != 0.:
...
res_earth_big[res_trans_big != 0]=0  # set zero to substation that don't have transformers connected to ground
adm_earth_big=res_earth_big.copy()
adm_earth_big[adm_earth_big != 0] = 1./(adm_earth_big[adm_earth_big != 0]) 
earthadmit = np.diag(adm_earth_big)
...
# Option 1: Basic 1 V/km geoelectric fields, use NA coordinates:
if testfield:
...
for k in range(0,1): 
    ...
    for i in range(0,nconnects):
            ...
            for p in range(0, number_steps_lon-1): 
    ...
    Nvoltage = np.zeros((nnodes_new, nnodes_new), dtype=npf)
    for l in range(nconnects):
        Nvoltage[nodefrom_big[l],nodeto_big[l]] = Vn_tot[l]
        Nvoltage[nodeto_big[l],nodefrom_big[l]] = -Vn_tot[l]
    
    # FOR SHIELD WIRES:
    # ===============================================================
    extra_list_N=[]
    for l in range(nconnects, nconnects_new,2):
        print(l)
        npylon = np.count_nonzero(pathlonsteps[mother_line_E[l-nconnects],:])
        print(npylon)
        #first shw
        Nvoltage[nodefrom_big[l],nodeto_big[l]] = ((-Vn_tot[mother_line_E[l-nconnects]])/(npylon+1))*(RG[l-nconnects]/Req[l-nconnects] + 1)
        extra_list_N.append(Nvoltage[nodefrom_big[l],nodeto_big[l]])
        Nvoltage[nodeto_big[l],nodefrom_big[l]] = ((Vn_tot[mother_line_E[l-nconnects]])/(npylon+1))*(RG[l-nconnects]/Req[l-nconnects] + 1)
        #second shw
        l=l+1;
        Nvoltage[nodefrom_big[l],nodeto_big[l]] = ((Vn_tot[mother_line_E[l-nconnects]])/(npylon+1))*(RG[l-nconnects]/Req[l-nconnects] + 1)
        Nvoltage[nodeto_big[l],nodefrom_big[l]] = ((-Vn_tot[mother_line_E[l-nconnects]])/(npylon+1))*(RG[l-nconnects]/Req[l-nconnects] + 1)
    ...
    Evoltage = np.zeros((nnodes_new, nnodes_new), dtype=npf)
    for l in range(nconnects):
        Evoltage[nodefrom_big[l],nodeto_big[l]] = Ve_tot[l]
        Evoltage[nodeto_big[l],nodefrom_big[l]] = -Ve_tot[l]
    ...
    for l in range(nconnects, nconnects_new,2):
        npylon = np.count_nonzero(pathlonsteps[mother_line_E[l-nconnects],:])
        #first shw
        Evoltage[nodefrom_big[l],nodeto_big[l]] = ((-Ve_tot[mother_line_E[l-nconnects]])/(npylon+1))*(RG[l-nconnects]/Req[l-nconnects] + 1)
        extra_list_E.append(Evoltage[nodefrom_big[l],nodeto_big[l]])
        extra_list_E2.append(Evoltage[nodefrom_big[l],nodeto_big[l]]/Ve_tot[mother_line_E[l-nconnects]])
        Evoltage[nodeto_big[l],nodefrom_big[l]] = ((Ve_tot[mother_line_E[l-nconnects]])/(npylon+1))*(RG[l-nconnects]/Req[l-nconnects] + 1)
        #second shw
        l=l+1;
        Evoltage[nodefrom_big[l],nodeto_big[l]] = ((Ve_tot[mother_line_E[l-nconnects]])/(npylon+1))*(RG[l-nconnects]/Req[l-nconnects] + 1)
        Evoltage[nodeto_big[l],nodefrom_big[l]] = ((-Ve_tot[mother_line_E[l-nconnects]])/(npylon+1))*(RG[l-nconnects]/Req[l-nconnects] + 1)
    ...
    # LP1985 eq. (14): J = V/R
    Nlinecurr = np.zeros((nnodes_new, nnodes_new), dtype=npf)
    for m in range(nnodes_new):
        for n in range(nnodes_new):
            if resis[m,n] > 0.:
    ...
    Elinecurr = np.zeros((nnodes_new, nnodes_new), dtype=npf)
    for m in range(nnodes_new):
        for n in range(nnodes_new):
            if resis[m,n] > 0.:
...
for k in range(0,len(global_results)):
    Isub_N = np.multiply(global_results[k][0],adm_earth_big)
    Isub_E = np.multiply(global_results[k][1],adm_earth_big)
    subs_results.append([Isub_N, Isub_E])
    Iline_N, Iline_E = np.zeros(nconnects_new), np.zeros(nconnects_new)
...
#                         A.P.
    ...
    for l in range(0,nconnects_new):
        m=nodefrom_big[l]
        n=nodeto_big[l]
        newval = 1/res_line_big[l]
        if not np.isinf(newval):
            # i_mn = j_mk + (v_n − v_m)y_nm 
            Iline_N[l] = current_sources_results[k][0][m,n] + np.dot((global_results[k][0][m]-global_results[k][0][n]),(1/res_line_big[l])) 
            Iline_E[l] = current_sources_results[k][1][m,n] + np.dot((global_results[k][1][m]-global_results[k][1][n]),(1/res_line_big[l])) 
        else:
            print(l)
            Iline_N[l] = 0
            Iline_E[l] = 0
