...
--> Uniform electric field implementation without explicit shield wire modelling <--
...
def input_output(X):
    ...
    testfield       = True  #uniform
    networkpath = 'Data/GRID.txt'
    connectionspath =  'Data/CONNECTIONS.txt'

    temp=networkpath.split('/')
    filename1=temp[-1]
    SavePath=networkpath[0:-len(filename1)]

    out_name         = 'uniform_Efield_'
    
    return testfield,networkpath,connectionspath,out_name,SavePath;
...
testfield,networkpath,connectionspath,out_name,SavePath      =input_output('x')
